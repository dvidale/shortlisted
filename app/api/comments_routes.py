...
def get_comments(id):
    try:
        ...
        referral_lst = [
            ...
            for shortlist in user_shortlists
        ]
        ...
        threads_unpacked = list(
            map(
                lambda thread: [comment.to_dict() for comment in thread],
                comment_threads,
            )
        )

        nonempty_threads = list(
            filter(lambda thread: len(thread) > 0, threads_unpacked)
        )

        if len(nonempty_threads) == 0:
            return {"message": "no messages yet"}, 404
        else:
            return nonempty_threads
    except:
        {"error": "There was a problem getting messages"}, 500
...
def add_comment():
    ...
    if comment_form.validate_on_submit():
        ...
        # update the comment recipient's received messages count
        ...
        newCommentFromDB = db.session.scalars(
            ...
            .where(Comment.referral_id == referral_id)
