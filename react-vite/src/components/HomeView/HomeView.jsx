...
import { getCommentThreads, getReceivedMsgsCount } from "../../redux/comments";
...
function HomeView() {
  ...
  const { centerPanel, setCenterPanel, leftPanel, setLeftPanel } =
    useContext(PanelViews);
  ...
  useEffect(() => {
    if (user) {
      ...
      const checkReferralMsgs = setInterval(() => {
      ...
      return () => clearInterval(checkReferralMsgs);
    }
  }, [user, dispatch]);
  ...
  const currentCommentThreads = useSelector(
    (state) => state.comments.comment_threads
  );
  ...
  useEffect(() => {
    ...
    if (user && threadHasComments) {
      const checkShortlistComments = setInterval(() => {
        dispatch(getCommentThreads(user.id));
      }, 10 * 1000);

      return () => clearInterval(checkShortlistComments);
    }
        ...
        <>
          ...
          <div id="search-and-my-shortlist-container">
            <button id="new-shortlist-btn" onClick={() => toggleFormView()}>
              <h1>New Shortlist</h1>
              <h1>{toggleSymbol ? <FaPlusCircle /> : <FaMinusCircle />}</h1>
            </button>
            ...
            <div
              ...
              style={{
                display:
                  leftPanel === "shortlist-search" ||
                  centerPanel === "shortlist-search"
                    ? "flex"
                    : "none",
              }}
            >
            ...
            {!isTabletOrMobile && (
              <div
                ...
                style={{
                  display: leftPanel === "my-shortlists" ? "flex" : "none",
                }}
              >
                <MyShortlists
          ...
          <div
            ...
            style={{
              display: centerPanel === "single-shortlist" ? "flex" : "none",
            }}
          >
            <SingleShortlistView
              setEditForm={setEditForm}
          ...
          <div
            ...
            className="center-panel"
            style={{
              display: centerPanel === "search-results" ? "flex" : "none",
            }}
          >
            <SearchResultsView
              user={user}
          ...
          <div
            id="recent-activity-view"
            className={"center-panel"}
            style={{
              display: centerPanel === "recent-activity" ? "flex" : "none",
            }}
          >
            <RecentActivityFeed />
          </div>
          ...
          <div
            id="referral-shortlist-view"
            className={"center-panel"}
            style={{
              display: centerPanel === "referral-shortlist" ? "flex" : "none",
            }}
          >
          ...
          <div
            ...
            className={"center-panel"}
            style={{
              display: centerPanel === "user-profile" ? "flex" : "none",
            }}
          >
