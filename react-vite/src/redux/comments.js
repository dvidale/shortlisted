...
export const getCommentThreads = (id) => async (dispatch) => {
  ...
  if (response.ok) {
    const data = await response.json();
    // TODO - try adding a conditional to only run the dispatch if the data value changed since the last fetch
...
const commentsReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case ADD_A_COMMENT: {
      const newState = { ...state };
      newState.comment_threads[action.payload["referral_id"]] = action.payload[
        "id"
      ] = action.payload;
      return newState;
    }
    case EDIT_A_COMMENT: {
      const newState = { ...state };
      const target_thread =
        newState.comment_threads[action.payload["referral_id"]];
    ...
    case CLEAR_DELETED_THREAD: {
      ...
      action.payload.forEach(
        (threadId) => delete newState.comment_threads[threadId]
      );
      return newState;
    }
    case RESET_COMMENTS: {
